...
function scarab(rootDir) {
	...
	configFiles.forEach(function(configFile) {
		Object.merge(app.config, require(path.join(configDir, configFile)));
	});
...
scarab.run = function(app, cb) {
	...
	scarab.server.listen(app.get('port'), function() {
		...
		app.init(function(err) {
			...
			console.log('\033[1mscarab server listening on port ' + app.get('port') + ' in ' + app.get('env') + ' mode.\033[0m');
...
scarab.uncache = function(app) {
	var serverPath = path.resolve(app.config.rootDir);
	for (var file in require.cache) {
		if (file.indexOf(serverPath) === 0) {
